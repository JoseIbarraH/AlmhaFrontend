<template>
  ...
  <div v-else>
    <section class="p-6 space-y-6 dark:bg-gray-950">
      ...
      <Statistics :total-object-title="$t('Dashboard.Procedure.Statistics.Total')" :total-object="stats?.total"
        ...
        :total-deactivated="stats?.totalDeactivated" :last-object-title="$t('Dashboard.Procedure.Statistics.Last')"
      ...
      <div class="bg-white rounded-lg shadow-md dark:bg-gray-900">
        <ProcedureGrid :data="paginate?.data ?? []"
          @status-updated="fetchProcedures(route.query.page ? Number(route.query.page) : 1)" @search="handleSearch" />
      </div>
  ...
  <NewProcedureModal :show="isOpen" max-width="lg" @close="closeModal" @refresh="refresh" />
