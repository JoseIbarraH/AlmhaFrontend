<template>
  ...
  <div class="flex justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
    <h2 class="text-lg font-semibold text-gray-800 dark:text-white">
      {{ $t('Dashboard.Procedure.List.Procedures') }}
    </h2>

    <Search
      :placeholder="$t('Dashboard.Procedure.List.Search')"
      @search="handleSearch"
      v-model="localSearch" />
  </div>
  ...
  <div v-if="data.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
    <article v-for="value in currentData()" :key="value.id" class="bg-white border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow duration-200
        ...
        <div class="flex-1">
          ...
          <div class="flex items-center gap-2">
            ...
            <span :class="[
              ...
              {{ value.status === 'active' ? $t('Dashboard.Procedure.List.Active') : $t('Dashboard.Procedure.List.Inactive')}}
            </span>
          </div>
        </div>
      ...
      <div class="mb-6 space-y-2">
        ...
        <div class="flex justify-between text-sm">
          <span class="text-gray-500 dark:text-gray-400">{{ $t('Dashboard.Procedure.List.LastUpdate') }}</span>
          <span class="text-gray-900 dark:text-gray-100">{{ value.updated_at }}</span>
        </div>
        <div class="flex justify-between text-sm">
          <span class="text-gray-500 dark:text-gray-400">{{ $t('Dashboard.Procedure.List.CreationDate') }}</span>
      ...
      <div class="flex gap-2">
        ...
        <button @click="handleEdit(value.id)" class="flex-1 px-4 py-2 text-sm font-medium text-blue-700 bg-blue-50 border border-blue-200 rounded-md hover:bg-blue-100 transition-colors duration-200
          ...
          {{ $t('Dashboard.Procedure.List.Edit') }}
        </button>
        ...
        <button :class="[
          ...
          {{ value.status === 'active'
            ? $t('Dashboard.Procedure.List.Deactivate')
            : $t('Dashboard.Procedure.List.Activate')}}
        </button>
  ...
  <div v-else class="flex flex-col items-center justify-center py-12 text-center bg-white dark:bg-gray-800">
    ...
    <h3 class="text-lg font-semibold text-gray-900 mb-2 dark:text-white">{{ $t('Dashboard.Procedure.List.NoResults') }}
    </h3>
    <p class="text-gray-500 dark:text-gray-400">{{ $t('Dashboard.Procedure.List.Try') }}</p>
  </div>
...
const handleEdit = (id: number) => {
  router.push({ name: 'dashboard.procedure.edit', params: { id } })
}
...
const confirmDelete = async () => {
  ...
  try {
    ...
    showNotification('error', t('Dashboard.Procedure.Validations.Error.Delete'), 4000)
  }
}
...
const handleToggleStatus = async (data: Data) => {
  ...
  try {
    ...
    showNotification('error', t('Dashboard.Procedure.Validations.Error.Status'), 4000)
