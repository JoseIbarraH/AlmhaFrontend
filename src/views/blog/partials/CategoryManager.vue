  ...
  <Modal :show="props.show" :max-width="props.maxWidth || 'lg'" @close="handleClose">
    <div class="p-6 bg-white dark:bg-gray-800">
      ...
      <div class="flex items-center justify-between mb-6">
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $t('Dashboard.Blog.Edit.CategoryData.Title') }}</h2>
      ...
      <div class="mb-6 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg border border-gray-100 dark:border-gray-700">
        <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-3">{{$t('Dashboard.Blog.Edit.CategoryData.New.Title')}}</h3>
        <div class="flex gap-2">
          <TextInput type="text" :placeholder="$t('Dashboard.Blog.Edit.CategoryData.New.Input')" v-model="form.title" />
          <PrimaryButton @click="addCategory"
            ...
            {{$t('Dashboard.Blog.Edit.CategoryData.New.Add')}}
          </PrimaryButton>
        </div>
      </div>
      ...
      <div class="mb-6">
        <div class="flex items-center justify-between mb-3">
          <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-200">{{ $t('Dashboard.Blog.Edit.CategoryData.List.Title') }}</h3>

          <!-- Buscador -->
          <div class="relative w-1/2">
            <input type="text" v-model="searchQuery" :placeholder="$t('Dashboard.Blog.Edit.CategoryData.List.Search')"
        ...
        <div class="space-y-2 max-h-96 overflow-y-auto custom-scrollbar">
            ...
            <div class="flex items-center gap-2">
              <template v-if="editingId === category.id">
                <button @click="saveEdit(category.id)"
                  ...
                  :title="$t('Dashboard.Blog.Edit.CategoryData.List.TitleSave')">
                ...
                <button @click="cancelEdit"
                  ...
                  :title="$t('Dashboard.Blog.Edit.CategoryData.List.TitleCancel')">
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                  </svg>
                </button>
              ...
              <template v-else>
                <button @click="startEdit(category)"
                  ...
                  :title="$t('Dashboard.Blog.Edit.CategoryData.List.TitleEdit')">
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <button @click="deleteCategory(category.id)"
                  ...
                  :title="$t('Dashboard.Blog.Edit.CategoryData.List.TitleDelete')">
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
          ...
          <div v-if="filteredCategories?.length === 0" class="text-center py-8 text-gray-500 dark:text-gray-400">
            {{ searchQuery ? $t('Dashboard.Blog.Edit.CategoryData.List.NotFound') : $t('Dashboard.Blog.Edit.CategoryData.List.Empty') }}
          </div>
        </div>
      </div>
      ...
      <div class="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
        <SecondaryButton @click="handleClose">
          {{$t('Dashboard.Blog.Edit.CategoryData.List.CloseButton')}}
        </SecondaryButton>
      </div>
    </div>
  </Modal>
