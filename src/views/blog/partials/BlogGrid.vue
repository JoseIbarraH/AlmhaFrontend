  ...
  <div v-if="data.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
    <article v-for="value in currentData()" :key="value.id" class="bg-white border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow duration-200
      ...
      <div class="flex gap-2">
        <button @click="handleEdit(value.id)" :disabled="!$can('update_blogs')" class="flex-1 px-4 py-2 text-sm font-medium text-blue-700 bg-blue-50 border border-blue-200 rounded-md hover:bg-blue-100 transition-colors duration-200
        ...
        ]" @click="handleToggleStatus(value)" :disabled="!$can('update_blogs_status')">
          {{ value.status === 'active' ? $t('Dashboard.Blog.List.Deactivate') : $t('Dashboard.Blog.List.Activate') }}
        </button>

        <button @click="openModal(value)" :disabled="!$can('delete_blogs')"
...
const confirmDelete = async () => {
  if (!auth.can('delete_blogs')) return
...
const handleToggleStatus = async (data: Data) => {
  if (!auth.can('update_blogs_status')) return
