  ...
  <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 dark:border-gray-600">
    ...
    <div v-else class="space-y-4">
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 max-h-[600px] overflow-y-auto pr-2
        ...
        <div v-for="(item, index) in carouselNavbar" :key="item.id || index" class="group relative bg-white rounded-lg overflow-hidden shadow-md hover:shadow-xl transition-all duration-300
          ...
          <div class="aspect-square relative overflow-hidden bg-gray-100 dark:bg-gray-900">
            <!-- Video -->
            <video v-if="isVideo(item)" :src="item.path" class="w-full h-full object-cover" muted playsinline
          ...
          <div class="absolute top-2 right-2 flex gap-2">
            <!-- Botón editar -->
            <button type="button" @click="emit('edit_item', item)" title="Edit image" class="p-2 bg-white/95 backdrop-blur-sm text-gray-700 hover:text-blue-600 hover:bg-blue-50
            ...
            <!-- Botón eliminar -->
            <button type="button" @click="emit('delete_item', item.id)" title="Delete image" class="p-2 bg-white/95 backdrop-blur-sm text-gray-700 hover:text-red-600 hover:bg-red-50
                     rounded-lg shadow-lg transition-all hover:scale-110
