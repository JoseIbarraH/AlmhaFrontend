<template>
  <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 dark:border-gray-600">
    ...
    <!-- Empty state -->
    <div v-if="!carouselImage || carouselImage.length === 0" class="py-12 text-center">
      ...
      <button type="button" @click="emit('create_item')" :disabled="!$can('update_design')"
        ...
        {{ $t('Dashboard.Design.ChooseCarouselImage.FirstImage') }}
      </button>
    </div>
    ...
    <div v-else class="space-y-4">
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 max-h-[600px] overflow-y-auto pr-2
        ...
        <div v-for="(item, index) in carouselImage" :key="item.id || index" class="group relative bg-white rounded-lg overflow-hidden shadow-md hover:shadow-xl transition-all duration-300
          ...
          <div class="aspect-square relative overflow-hidden bg-gray-100 dark:bg-gray-900">
            <!-- Video -->
            <video v-if="isVideo(item)" :src="item.path" class="w-full h-full object-cover" muted controls playsinline
              preload="metadata">
              {{ $t('Dashboard.Design.Backgrounds.VideoNotSupported') }}
            </video>

            <!-- Imagen -->
            <img v-else :src="item.path" :alt="item.title || `Media ${index + 1}`"
            ...
            <div v-if="!isVideo(item)"
              class="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all duration-300"></div>
          </div>

          <!-- Información de la media (opcional) -->
          <div v-if="(item.title || item.subtitle) && item.type === 'image'" class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-3
          ...
          <div class="absolute top-2 right-2 flex gap-2">
            <!-- Botón editar -->
            <button type="button" @click="emit('edit_item', item)" title="Editar" class="p-2 bg-white/95 backdrop-blur-sm text-gray-700 hover:text-blue-600 hover:bg-blue-50
                     ...
                     disabled:opacity-50 disabled:cursor-not-allowed" :disabled="!$can('update_design')"
              aria-label="Editar">
              <LucideSquarePen class="w-4 h-4" />
            </button>

            <!-- Botón eliminar -->
            <button type="button" @click="emit('delete_item', item.id)" title="Eliminar" class="p-2 bg-white/95 backdrop-blur-sm text-gray-700 hover:text-red-600 hover:bg-red-50
                     rounded-lg shadow-lg transition-all hover:scale-110
