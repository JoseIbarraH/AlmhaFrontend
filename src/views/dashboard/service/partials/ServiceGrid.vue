<template>
  ...
  <div v-if="data.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
    <article v-for="value in currentData()" :key="value.id"
      ...
      <div class="mb-6 space-y-2">
        ...
        <div class="flex justify-between text-sm">
          <span class="text-gray-500">{{ $t('Dashboard.Service.List.LastUpdate') }}</span>
          <span class="text-gray-900">{{ value.updated_at }}</span>
        </div>
        <div class="flex justify-between text-sm">
          <span class="text-gray-500">{{ $t('Dashboard.Service.List.CreationDate') }}</span>
  ...
  <div v-else class="flex flex-col items-center justify-center py-12 text-center">
    ...
    <h3 class="text-lg font-semibold text-foreground mb-2">{{ $t('Dashboard.Service.List.NoResults') }}</h3>
  </div>
        ...
        <div class="flex items-start gap-4">
          ...
          <div class="flex-1">
            <h2 class="text-xl font-bold text-gray-900 mb-1">
              {{ $t('Dashboard.Service.Delete.ConfirmTitle') }}
            </h2>
            <p class="text-sm text-gray-600">
              {{ $t('Dashboard.Service.Delete.ConfirmSubtitle') }}
            </p>
          </div>
      ...
      <div class="p-6 bg-white">
        <div class="mb-6">
          ...
          <div class="mt-4 p-4 bg-red-50 border border-red-100 rounded-lg">
            <p class="text-sm font-medium text-red-800 mb-2">
              {{ $t('Dashboard.Service.Delete.Consequences.Title') }}
            </p>
            <ul class="text-sm text-red-700 space-y-1.5">
              <li class="flex items-start gap-2">
                ...
                <span>{{ $t('Dashboard.Service.Delete.Consequences.First') }}</span>
              </li>
              <li class="flex items-start gap-2">
                <svg class="w-4 h-4 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                  <path fill-rule="evenodd"
                ...
                <span>{{ $t('Dashboard.Service.Delete.Consequences.Second') }}</span>
              </li>
              <li class="flex items-start gap-2">
                <svg class="w-4 h-4 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                  <path fill-rule="evenodd"
                ...
                <span>{{ $t('Dashboard.Service.Delete.Consequences.Third') }}</span>
              </li>
            </ul>
          </div>
        </div>
        ...
        <div class="flex flex-col-reverse sm:flex-row sm:justify-end gap-3">
          <button @click="closeModal"
            ...
            {{ $t('Dashboard.Service.Delete.Cancel') }}
          </button>
          ...
          <button @click="confirmDelete"
            ...
            {{ $t('Dashboard.Service.Delete.Delete') }}
          </button>
        </div>
      </div>
    </div>
